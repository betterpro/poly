...
def _position_payload(inventory, position, mark_price: float | None) -> dict:
...
async def _load_books_and_trades(
    ...
    *,
    limit: int = 40,
) -> tuple[dict[str, OrderBook], dict[str, list]]:
    candidates = sorted(markets, key=lambda market: (market.liquidity, market.volume), reverse=True)[:limit]
...
async def run_once() -> None:
    ...
    try:
            ...
            for market in selected:
                ...
                if signal and signal.bid_price and signal.ask_price:
                    ...
                    await execution.create_order(
                        market.condition_id, Side.BUY, signal.bid_price, signal.size, market.yes_token_id
                    )
                    await execution.create_order(
        ...
        mark_prices = {
            market.condition_id: mark
            for market in selected
            if (mark := _mark_price(market, books.get(market.condition_id))) is not None
        }
