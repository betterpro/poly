...
from polymarket_mm_bot.models import BotOrder, Market, OrderBook, OrderStatus
...
class RiskEngine:
    ...
    def check_order(self, order: BotOrder, open_orders: list[BotOrder]) -> RiskDecision:
        ...
        if self.inventory.total_exposure() >= self.settings.max_total_exposure:
            return self._deny("max_total_exposure", order.market_id)
        position = self.inventory.get_position(order.market_id)
        if abs(position.net_yes) + order.size > self.settings.max_position_per_market:
